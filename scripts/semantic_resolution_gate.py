...
def validate_manifest(
    ...
    *,
    ...
    raw_outputs = manifest.get("current_cycle", {}).get("raw_outputs", {})
    raw_final_ref = raw_outputs.get("final_blind") if isinstance(raw_outputs, dict) else None
    raw_candidate_index: dict[str, dict[str, Any]] = {}
    if not isinstance(raw_final_ref, dict) or not _nonempty(raw_final_ref.get("path")):
        errors.append("current_cycle.raw_outputs.final_blind.path is required")
    else:
        raw_path = repo_root / str(raw_final_ref.get("path"))
        raw_final = _load_json(raw_path, errors, "sealed final_blind raw output")
        raw_candidate_index = _index_by_id(
            raw_final.get("independent_candidates"),
...
def _canonical_audit_paths(repo_root: Path = REPO_ROOT) -> list[Path]:
    ...
    for path in sorted((repo_root / "audits").rglob("*.json")):
        ...
        if any(part in {"runs", "history"} for part in relative.parts):
            continue
        if path.name == "escaped_defect_taxonomy.json":
            continue
        paths.append(path)
    return paths
...
def command_validate_changed(args: argparse.Namespace) -> int:
    ...
    for raw_path in _changed_files(args.base, args.head):
        ...
        if raw_path.startswith("audits/") and raw_path.endswith(".json"):
            ...
            if any(part in {"runs", "history"} for part in relative.parts):
                continue
            if Path(raw_path).name == "escaped_defect_taxonomy.json":
                continue
