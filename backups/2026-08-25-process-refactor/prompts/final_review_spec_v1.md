...
調整役は最終審査担当へ過去の判断を見せずに、対応する `entry_workflow_guard` の全体時間・検索query・候補page・heartbeat budgetを適用する。外部資料を確認するbatchはattempt数を事前記録し、guardの終了コード2では `PASS` を出さず、盲検または照合の保存済み段階とblockerをcommit・pushして安全停止する。budgetを満たすために盲検独立性や合否基準を緩めない。
