...
`premises` の「建物・敷地」という意味は、単に「前提が複数ある」ことから生じたのではない。法律文書で先に述べられた事項や、譲渡文書で記載された対象を *premises* と呼んだ歴史を経て、そこに記載された土地・建物そのものを指すようになった。現代英語では、建物・敷地を指すときは複数形 `premises` が標準的な形である。一方、法務・規制・不動産管理などの限定された文脈では、単一の場所を指して単数の `a premise` / `one premise` も使われる。したがって「常に複数形」や「単数形は常に誤り」と一般化せず、一般用法の標準形と専門文脈の単数形を分けて覚えるのが実用的である。  
...
`premises` の「建物・敷地」という用法は歴史的には同じ語から発達したが、現代話者にとって上のコアイメージから直接推測しにくい定着義なので、語義4として別に意識する。ただし、一般用法の標準形は複数形 `premises` であり、限定的な単数形 `a premise` を一律に誤りとはしない。 名詞の「前提」と「前提命題」は同じ中心概念を共有し、論理学では後者の専門的な枠組みとして現れる。動詞の二つの説明も、同じ「前提を置く」という核を目的語の取り方の違いから整理したものである。  
...
【日本語訳・定義】人・会社・組織などが所有・占有・使用する土地、建物、建物の一部、およびそれに付随する敷地。会社・店舗・学校・病院・工場などの場所に特によく使うが、住宅や賃貸物件などにも使える。現代英語ではこの意味を通常 `premises` という複数形で表す。単一の施設でもこの複数形を使うのが一般的だが、法務・規制・不動産管理などの限定された文脈では、単数の `a premise` / `one premise` も見られる。一般文では `a building`、`a property`、`a site` のほうが自然なことが多い。  
