...
`premises` の「建物・敷地」という意味は、単に「前提が複数ある」ことから生じたのではない。法律文書で先に述べられた事項や、譲渡文書で記載された対象を *premises* と呼んだ歴史を経て、そこに記載された土地・建物そのものを指すようになった。現代ではこの不動産義は複数形 `premises` として定着しているため、語源上のつながりより用法を独立して覚えるほうが実用的である。  

＃語形成

・premises：`premise` の通常の複数形。また、複数形の形に固定して「（事業所などの）建物・敷地」を表す。後者でも文法上は複数扱いが基本で、`the premises are ...` のように使う。  
...
`premise` の中心は、「後に続く結論・判断・構成を支えるものを、先に置いて土台にする」である。議論では命題を先に置き、判断では仮定を土台にし、作品では物語を成立させる基本設定を置く。動詞では、その土台の上に理論や主張を組み立てることを表す。  
・議論・判断の土台として置く考え → 「前提、仮定」（語義1）  
・作品を成立させる土台の設定 → 「基本設定、着想」（語義2）  
・論理学で結論を導くために置く命題 → 「前提命題」（語義3）  
・理論・主張などをある考えの上に置く → 「～を…に基づかせる」（語義5）  
`premises`「建物・敷地」（語義4）は歴史的には同じ語から発達したが、現代話者にとって上のコアイメージから直接推測しにくい定着義なので、別に覚える。  
...
【日本語訳・定義】映画、小説、ドラマ、ゲーム、企画などを成立させる基本的な状況・設定・アイデア。細かな筋書き全体ではなく、「そもそも何を題材に、どんな状況から展開する作品・企画なのか」という土台を指す。  
【頻度】〈7/10〉  
...
【語法・注意】この語義の `premise` は `plot` と同じではない。`premise` は物語を生む基本状況・アイデアで、`plot` は出来事がどのような順序・因果で展開するかという筋書きに焦点がある。`concept` はさらに広く、作品以外の発想や概念にも使える。  
...
【日本語訳・定義】論証・推論において、結論を導くための出発点として置かれる命題。三段論法では `major premise`「大前提」、`minor premise`「小前提」のように呼ぶ。一般語義1と連続しているが、ここでは論証の構成要素としてより厳密に用いる。  
【頻度】〈6/10〉  
...
・`major premise`  
用途: 伝統的な三段論法で、一般的な規則・命題となる大前提を指す。  
例: In the syllogism, “All metals conduct electricity” functions as the major premise.  
訳: その三段論法では、「すべての金属は電気を通す」が大前提として機能する。  

・`minor premise`  
用途: 三段論法で、個別の対象を大前提に結びつける小前提を指す。  
例: The minor premise states that copper is a metal.  
訳: 小前提は、銅は金属であると述べている。  
...
4. 【名詞・複数形】（会社・組織などが使用する）建物・敷地、構内

【日本語訳・定義】会社、店舗、学校、病院、工場などが所有・占有・使用する土地と、その上の建物・施設。現代英語ではこの意味を通常 `premises` という複数形で表し、単数 `a premise` を「一つの建物・敷地」の意味では普通使わない。  
【頻度】〈8/10〉  
【レジスター/領域】標準～やや形式的。規則、契約、警告表示、警備、保険、不動産、事業運営で非常によく使う。  
...
【語法・注意】この意味の `premises` は見た目が複数形で、標準的には複数動詞を取る。`These premises are ...`、`the premises have ...` のように考える。単数の建物を指したい場合も、通常は `a premise` ではなく `a building`、`a property`、`a site` などを使う。  
`premises` は建物だけに限定されず、そこに付随する土地・敷地を含み得る。したがって、日本語訳は文脈に応じて「建物」「施設」「敷地」「構内」「事業所」などを選ぶ。語義1の複数形 `premises`「複数の前提」と綴り・発音が同じなので、`on/off the premises` や事業・警備文脈を手掛かりに区別する。  
...
定義: 所有される土地・建物、不動産。  
頻度: 〈10/10〉  
違い: `property` は所有物・不動産そのものに焦点があり、個人住宅にも広く使える。`premises` は会社・組織などが占有・使用している場所として捉えることが多い。  
...
【日本語訳・定義】理論、議論、計画、判断などを、ある考え・仮定が真または受け入れられるものとして土台にして組み立てる。現代英語では、とくに受動態 `be premised on/upon ...`「～を前提としている、～に基づいている」が重要である。  
【頻度】〈6/10〉  
【レジスター/領域】形式的。学術、政策、法律、ビジネス分析、評論で使う。日常会話では `base ... on ...` のほうが一般的。  
【文法パターン】`premise 〈理論・主張・計画〉 on/upon 〈考え・仮定〉`＝理論などを考え・仮定に基づかせる／`〈理論・主張・計画〉 be premised on/upon 〈考え・仮定〉`＝理論などが考え・仮定を前提としている／`premise that ...`＝～だと前提として述べる・仮定する（まれ・形式的）  
【コロケーション】
