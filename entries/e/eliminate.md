...
16世紀に、ラテン語 eliminare「戸外へ追い出す、排除する」の過去分詞 eliminatus から英語に入った。eliminare は ex「外へ」と limen「敷居、境界」を含み、もともとは「敷居の外へ出す」という発想を表した。現在の「不要なものを外へ出す」「候補から外す」「式から未知量を消す」という意味には、この「外へ押し出す」という核が共通している。  
同語源・関連語として、limit「限界、制限」や limen「境界、敷居」がある。ただし、eliminate と limit は現代の意味を置き換えられる関係ではない。  
...
【語法・注意】この語義では `eliminate` は「検討して不適切・不可能と判断する」という含みを持つことがある。`exclude` は範囲・参加・含有から外すことに焦点があり、`rule out` は可能性や説明を否定して検討対象から外すことに特に自然である。`eliminate a suspect` だけでは、捜査から外すのか、競技から敗退させるのか、殺すのかが文脈依存になるため、`as a suspect`、`from the investigation` などを添えると誤解が少ない。  
...
【日本語訳・定義】人体や動物の体が、老廃物・不要な物質などを体外へ出す。医学・生理学の用法で、対象を単に「なくす」のではなく、生体の排泄・排出過程に焦点がある。  

【頻度】〈5/10〉  

【レジスター/領域】医学・生理学、健康・薬理の説明。一般向けの記事でも toxins、waste、drugs などと共に使われる。  

【文法パターン】`the body/eliminates 〈waste/toxins〉`＝体が〈老廃物・毒素〉を排出する／`eliminate 〈物質〉 from the body`＝〈物質〉を体から排出する／`〈物質・薬〉 be eliminated from the body`＝〈物質・薬〉が体から排出される。生理学では自動詞的に「排泄する」と説明されることもあるが、学習上は目的語を置く他動詞構文が基本である。  
...
用途: 腎結石などを排出する。  
例: The patient passed the stone without surgery and eliminated it naturally.  
訳: その患者は手術をせずに結石を排出した。  

【語法・注意】この語義では、対象は通常 waste、toxins、drugs、stones のような体内の物質である。`excrete` は生理学的な排泄をより直接に表し、`eliminate` は「体内からなくす」という結果に焦点を置く。健康情報で `eliminate toxins` と言う場合、何を毒素と呼ぶかは文脈に依存するため、一般化して「体内のすべての毒を除く」と解釈しない。  
