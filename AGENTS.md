...
- 通常チェック担当は指摘候補の判定、必要な本文修正、採用が1件以上ある場合の全文再検査を行い、全監査対象、独立棚卸し、根拠台帳、コールドレビューと解決結果を監査ファイルへ記録してstatus `review_ready`、checked `false` まで進める。
- 通常チェック担当とコールドレビュー担当のどちらとも異なる最終審査担当が `prompts/final_review_spec_v1.md` に従って全件を判定し、`PASS` の場合だけ調整役がstatus `checked`、checked `true` へ機械的に同期する。
...
- `scripts/content_audit.py build` で本文から監査対象一覧を生成し、通常チェック担当は全targetを個別判定する。独立棚卸し候補と収録・除外理由、根拠台帳も監査ファイルへ記録し、内部確認だけで完了させない。
...
- 元の作業実行は、各問題候補を現行チェック仕様と信頼できる資料で個別に検証し、`採用`、`不採用`、`保留` のいずれかに判定する。コールドレビューの指摘を自動的に正解として採用しない。
...
- 必要な候補判定・修正と、採用がある場合の全文再検査を終えて保留が0件になった後、通常チェック担当はfront matterとqueueをstatus `review_ready`、checked `false` に同期する。第三者の最終審査担当が全target、全独立候補、全finding、根拠台帳を個別判定し、本文ハッシュに結び付いた `PASS` を監査ファイルへ記録した場合だけ、調整役がstatus `checked`、checked `true` に同期する。

## 第三者の最終審査

- 最終審査は、通常チェックとコールドレビューのどちらも担当していない文脈非継承の別実行で行う。
- 最終審査担当は `prompts/final_review_spec_v1.md` だけを最初から最後まで読み、最新版記事、対応する監査ファイル、根拠資料を確認する。
- 本文から自動生成された全target、通常チェック側の全独立棚卸し候補、コールドレビューの全findingを一つずつ判定し、未判定を残さない。
- 高リスクtargetと全独立候補について、根拠が存在するだけでなく、該当箇所が実際に主張を支持することを確認する。
- 最終審査担当は本文を修正せず、`PASS` または `REJECT` とblockerだけを監査ファイルへ記録する。自分で直した版へ自分で合格を出さない。
- 通常チェック、コールドレビュー、最終審査の三者の担当識別子が相互に異なることを監査ファイルへ記録する。同一実行の役割切替を三者分離の代用にしない。
- `REJECT` の場合は通常チェック側が修正し、監査対象一覧、本文ハッシュ、通常チェック結果、根拠台帳を更新して、別実行の最終審査を再度受ける。コールドレビューは記事を全面再構築した場合を除き1回のままとする。
...
- `scripts/content_audit.py`: 記事から汎用的な監査対象一覧と本文ハッシュを生成し、通常チェック・独立棚卸し・根拠台帳・コールドレビュー・第三者最終審査の全件完了を検証する。
