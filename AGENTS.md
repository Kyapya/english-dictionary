...
- 自動チェック後は、後述の「チェック後の必須コールドレビュー」を文脈を継承しない1回の独立実行で行い、指摘候補の判定、必要な本文修正、採用が1件以上ある場合の全文再検査まで完了させる。
...
- 通常チェックだけを理由に `checked: true` にせず、文脈を継承しない1回の必須コールドレビュー、指摘の判定・修正、採用が1件以上ある場合の全文再検査を続ける。
...
- コールドレビュー担当へ渡すのは、front matterを除いた通常チェック後の最新版本文と、`英単語解説として問題がないか、記事全体を横断して徹底的に走査し、内容上の問題を前提なしで指摘してください。各文の正誤だけでなく、語義の分け方・境界・重複と、学習者が説明から誤った一般化をしないかも確認してください。また、断定的な主張に対して反例を探すことで記述に問題が無いか確認をしてください。` という短い依頼だけとする。
...
- 担当は記事を直接修正せず、問題候補ごとに対象箇所、理由、可能なら確認根拠を返す。書式やプロジェクト固有ルールの最終判断は行わない。問題候補がなければ「問題候補なし」と明記する。
...
- ユーザーが「`<見出し語>`を辞書に追加してPRを作成して」のように依頼した場合、本文生成、独立チェック、文脈を継承しない1回のコールドレビュー、指摘の判定・修正、採用修正がある場合の全文再検査、形式検証、queue・logs更新、専用ブランチへのcommit・push、Pull Request作成、検証成功後のマージ、Notion同期成功の確認までを一つの標準作業として扱う。
...
- 通常チェック後は、文脈を継承しない1回の独立実行へ最新版本文と短い依頼だけを渡し、指摘候補の判定、必要な修正、採用が1件以上ある場合の全文再検査を完了する。
