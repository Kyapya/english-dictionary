...
- まず現行完全版の生成仕様を読み、本文を書く前に語義・構文棚卸し、必須構文マトリクス、対応づけ、双方向照合を内部で行う。
...
- `scripts/import_to_notion.py`: `prompts/notion_spec_v1.md` に従い、checked/final記事をNotionの新規ページへ追加する。同じ見出し語の既存ページがあっても更新・削除せず、同期ごとに別ページを作成する。GitHub上では既存のMarkdownを最新版へ上書きする。
