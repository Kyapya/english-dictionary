...
- 自動チェック後は、後述の「チェック後の必須コールドレビュー」を独立した実行で行い、指摘候補の判定・必要な本文修正・全文再検査まで完了させる。
- コールドレビュー後の再検査まで問題なく完了した場合だけ、front matter の `checked: true` とqueueのstatus `checked` を設定する。
...
- 通常チェックだけを理由に `checked: true` にせず、必須コールドレビュー、指摘の判定・修正、全文再検査を続ける。
...
- コールドレビュー担当へ渡すのは、front matterを除いた通常チェック後の最新版本文と、`〈見出し語〉の英単語解説として問題がないか、内容上の問題を前提なしで指摘してください。` という短い依頼だけとする。
...
- 採用修正後は、最新版全文に対して `prompts/check_spec_v5.md` の内容監査、双方向照合、発音監査、書式監査を再実行し、`scripts/validate_entry.py` と `scripts/validate_repository.py` と全単体テストを再実行する。変更箇所だけの確認で済ませない。
- 再検査で新たな修正が必要になった場合は、修正と全文再検査を問題がなくなるまで繰り返す。再検査中にコールドレビューの判定を既定の正解として扱わない。
- 全候補の判定、採用修正、保留ゼロ、全文再検査、形式検証、queue・logs同期がすべて完了した場合だけ、statusを `checked`、checkedを `true` にする。
...
- ユーザーが「`<見出し語>`を辞書に追加してPRを作成して」のように依頼した場合、本文生成、独立チェック、コールドレビュー、指摘の判定・修正、全文再検査、形式検証、queue・logs更新、専用ブランチへのcommit・push、Pull Request作成、検証成功後のマージ、Notion同期成功の確認までを一つの標準作業として扱う。
...
- 通常チェック後は、文脈を継承しない独立実行へ最新版本文と短い依頼だけを渡してコールドレビューを行い、指摘候補の判定・必要な修正・全文再検査を完了する。
