...
- 自動チェック後は、後述の「チェック後の必須コールドレビュー」を独立した実行で行い、指摘候補の判定・必要な本文修正・問題候補が1件以上ある場合の全文再検査まで完了させる。
...
- 通常チェックだけを理由に `checked: true` にせず、必須コールドレビュー、指摘の判定・修正、問題候補が1件以上ある場合の全文再検査を続ける。
- 修正不要の場合も、logsに「通常チェックでは修正不要」と明記し、その後のコールドレビュー結果と判定を追記する。

## チェック後の必須コールドレビュー

- 生成後の自動チェックとユーザー指示による個別チェックのどちらでも、`prompts/check_spec_v5.md` による通常チェックの直後にコールドレビューを必ず行う。
- コールドレビュー担当は、新しい会話または文脈を継承しない独立実行として開始する。同じ会話内で役割だけを切り替えた自己再評価を代用にしない。
- コールドレビュー担当へ渡すのは、front matterを除いた通常チェック後の最新版本文と、`英単語解説として問題がないか、内容上の問題を前提なしで指摘してください。` という短い依頼だけとする。
- `AGENTS.md`、生成仕様、チェック仕様、生成過程、通常チェックの結果、以前の指摘、差分、queue、logs、status、`checked` 値、既知の懸念をコールドレビュー担当へ渡さない。担当は必要に応じて辞書、コーパス、法令などの外部資料を自ら照合してよいが、プロジェクト内の判断履歴は参照しない。
- コールドレビュー担当は記事を直接修正せず、問題候補ごとに対象箇所、理由、可能なら確認根拠を返す。書式やプロジェクト固有ルールの最終判断は行わない。
...
- コールドレビューが問題候補を1件以上返した場合は、採用が0件でも、最新版全文に対して `prompts/check_spec_v5.md` の内容監査、双方向照合、発音監査、書式監査を再実行する。変更箇所だけの確認で済ませない。
- 問題候補が1件以上ある場合の再検査で新たな修正が必要になったときは、修正と全文再検査を問題がなくなるまで繰り返す。再検査中にコールドレビューの判定を既定の正解として扱わない。
- 問題候補が0件の場合は全文再検査を省略し、logsに「コールドレビューでは問題候補なし」と記録する。ただし、最終状態に対する機械検証は省略しない。
- 必要な候補判定・修正・全文再検査を終えて保留が0件になった後、front matterとqueueをstatus `checked`、checked `true` に同期し、logsを更新する。その最終状態で `scripts/validate_entry.py`、`scripts/validate_repository.py`、全単体テストを実行し、すべて成功した場合だけchecked状態を確定する。失敗した場合はstatusを `needs_review`、checkedを `false` に戻し、理由をqueueのnotesとlogsへ記録する。
...
- ユーザーが「`<見出し語>`を辞書に追加してPRを作成して」のように依頼した場合、本文生成、独立チェック、コールドレビュー、指摘の判定・修正、問題候補がある場合の全文再検査、形式検証、queue・logs更新、専用ブランチへのcommit・push、Pull Request作成、検証成功後のマージ、Notion同期成功の確認までを一つの標準作業として扱う。
...
- 通常チェック後は、文脈を継承しない独立実行へ最新版本文と短い依頼だけを渡してコールドレビューを行い、指摘候補の判定・必要な修正・問題候補が1件以上ある場合の全文再検査を完了する。
