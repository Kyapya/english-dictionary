...
- 自動チェック後は、後述の「チェック後の必須コールドレビュー」を相互に結果を見せない2回の独立実行で行い、両方の指摘候補の統合・判定、必要な本文修正、採用が1件以上ある場合の全文再検査まで完了させる。
- コールドレビュー以降の必要工程を完了した後、front matterとqueueを最終状態へ同期してから機械検証し、すべて成功した場合だけ `checked: true` とstatus `checked` を確定する。
...
- 通常チェックだけを理由に `checked: true` にせず、相互に結果を見せない2回の必須コールドレビュー、両方の指摘の統合・判定・修正、採用が1件以上ある場合の全文再検査を続ける。
- 修正不要の場合も、logsに「通常チェックでは修正不要」と明記し、その後のコールドレビュー結果と判定を追記する。

## チェック後の必須コールドレビュー

- 生成後の自動チェックとユーザー指示による個別チェックのどちらでも、`prompts/check_spec_v5.md` による通常チェックの直後に、同一の最新版本文を対象とするコールドレビューを2回必ず行う。
- 第1レビューと第2レビューは、新しい会話または文脈を継承しない別々の独立実行として開始し、互いの出力を見せない。同じ会話内で役割だけを切り替えた自己再評価や1回だけのレビューを代用にしない。
- 両方のコールドレビュー担当へ渡すのは、front matterを除いた同一の通常チェック後最新版本文と、`英単語解説として問題がないか、記事全体を横断して内容上の問題を前提なしで指摘してください。各文の正誤だけでなく、語義の分け方・境界・重複と、学習者が説明から誤った一般化をしないかも確認してください。` という短い依頼だけとする。
- `AGENTS.md`、生成仕様、チェック仕様、生成過程、通常チェックの結果、以前の指摘、差分、queue、logs、status、`checked` 値、既知の懸念、他方のコールドレビュー結果を担当へ渡さない。担当は必要に応じて辞書、コーパス、法令などの外部資料を自ら照合してよいが、プロジェクト内の判断履歴は参照しない。
- 各担当は記事を直接修正せず、問題候補ごとに対象箇所、理由、可能なら確認根拠を返す。書式やプロジェクト固有ルールの最終判断は行わない。問題候補がなければ各担当がそれぞれ「問題候補なし」と明記する。
- 元の作業実行は、両方の問題候補を統合する。同じ問題の言い換えはまとめてもよいが、片方だけが挙げた候補を捨てず、各候補を現行チェック仕様と信頼できる資料で個別に検証して `採用`、`不採用`、`保留` のいずれかに判定する。
...
- 採用が0件の場合は内容上の全文再検査を省略する。両方のレビューで問題候補が0件ならlogsに「2回のコールドレビューでは問題候補なし」、問題候補はあるが採用0件なら「コールドレビューの採用0件につき全文再検査省略」と記録する。保留があれば従来どおり `needs_review` とする。ただし、最終状態に対する機械検証は省略しない。
- 2回のうち一方でも未実施、文脈や他方の出力を継承、または候補未統合・未判定なら、コールドレビュー完了とみなさず `needs_review`、`checked: false` にする。
- 必要な候補判定・修正と、採用がある場合の全文再検査を終えて保留が0件になった後、front matterとqueueをstatus `checked`、checked `true` に同期し、logsを更新する。その最終状態で `scripts/validate_entry.py`、`scripts/validate_repository.py`、全単体テストを実行し、すべて成功した場合だけchecked状態を確定する。失敗した場合はstatusを `needs_review`、checkedを `false` に戻し、理由をqueueのnotesとlogsへ記録する。
## 仕様バージョンの移行
...
- ユーザーが「`<見出し語>`を辞書に追加してPRを作成して」のように依頼した場合、本文生成、独立チェック、相互に結果を見せない2回のコールドレビュー、両方の指摘の統合・判定・修正、採用修正がある場合の全文再検査、形式検証、queue・logs更新、専用ブランチへのcommit・push、Pull Request作成、検証成功後のマージ、Notion同期成功の確認までを一つの標準作業として扱う。
...
- 通常チェック後は、相互に結果を見せない2回の文脈非継承独立実行へ同一の最新版本文と短い依頼だけを渡し、両方の指摘候補の統合・判定、必要な修正、採用が1件以上ある場合の全文再検査を完了する。
