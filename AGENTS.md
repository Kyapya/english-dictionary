...
- 自動チェック後は、後述の「チェック後の必須コールドレビュー」を独立した実行で行い、指摘候補の判定・必要な本文修正・採用が1件以上ある場合の全文再検査まで完了させる。
...
- 通常チェックだけを理由に `checked: true` にせず、必須コールドレビュー、指摘の判定・修正、採用が1件以上ある場合の全文再検査を続ける。
...
- コールドレビューの指摘を1件以上 `採用` した場合は、修正版の最新版全文に対して `prompts/check_spec_v5.md` の内容監査、双方向照合、発音監査、書式監査を再実行する。変更箇所だけの確認で済ませない。
...
- ユーザーが「`<見出し語>`を辞書に追加してPRを作成して」のように依頼した場合、本文生成、独立チェック、コールドレビュー、指摘の判定・修正、採用修正がある場合の全文再検査、形式検証、queue・logs更新、専用ブランチへのcommit・push、Pull Request作成、検証成功後のマージ、Notion同期成功の確認までを一つの標準作業として扱う。
...
- 通常チェック後は、文脈を継承しない独立実行へ最新版本文と短い依頼だけを渡してコールドレビューを行い、指摘候補の判定・必要な修正・採用が1件以上ある場合の全文再検査を完了する。
