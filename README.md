...
4. 通常チェック担当が現行完全版 `prompts/check_spec_v5.md` だけを全文読み、生成時の分類を再利用しないゼロベース監査を行う。`scripts/content_audit.py build` で記事から全targetと記事横断の全relationを生成し、各対象の判定、独立棚卸し候補、収録・除外理由、主張単位の根拠リンク、実行履歴を `audits/` へ外部化する。この担当は最終合否を決めない。
5. 文脈を継承しない新しい会話または独立実行へ、front matterを除いた最新版本文と `prompts/check_spec_v5.md` に記載された正本の短い依頼だけを渡し、コールドレビューを1回行う。生成仕様、チェック仕様、過去の指摘、差分、queue、logs、既存判定は渡さない。コールドレビューは仕様が想定していない問題候補の発見を担当し、本文修正、体系的な全件保証、最終合否は担当しない。
...
8. 通常チェック担当とコールドレビュー担当のどちらとも異なる実行が、`prompts/final_review_spec_v1.md` と修正後本文だけから盲検の独立棚卸しと問題探索を行い、`scripts/content_audit.py seal-blind` で出力を固定する。その後に監査記録と根拠を開き、両者の棚卸しを双方向比較し、全target・relation・候補・findingを一つずつ判定する。本文は修正しない。
...
12. GitHub ActionsはAI生成・内容判断を行わず、PR内の記事形式、queueとの整合、本文ハッシュ、三者のrun・context分離、盲検出力の固定、全target・relation・棚卸し比較・findingの判定、主張単位の根拠リンク、最終 `PASS` または完了した `REJECT` の整合を再検証する。監査notesの文面一致だけでは拒否しない。
13. 検証成功後、エージェントがPull Requestを `main` へマージする。利用者がレビュー待ちを明示した場合だけ、マージせず停止する。
14. `.github/workflows/sync-notion.yml` がマージされた完成記事をNotionへ同期する。
...
- `prompts/check_spec_v5.md`: 校正、独立構文マトリクス、ゼロベース棚卸し、品詞・構文境界監査、文脈非継承のコールドレビュー、指摘の判定・修正、全文再検査、本文書式、通常チェック側の引き渡し条件をすべて含む現行完全版チェック仕様。通常チェックと後続判定・再検査を管理する実行だけが読み、コールドレビュー担当には渡さない。
- `prompts/final_review_spec_v1.md`: 通常チェック・コールドレビューとは別の実行が、盲検の独立棚卸しを先に固定し、通常チェックの棚卸しとの比較、全target・relation、主張単位の根拠リンク、コールドレビュー解決結果を個別判定して最終合否を出す仕様。
...
- `scripts/content_audit.py`: 本文から全target・relationと本文ハッシュを生成し、実行履歴、盲検最終棚卸しの固定、棚卸し比較、全件判定、主張単位の根拠リンク、コールドレビューの構造化解決、最終 `PASS` または `REJECT` を検証する。
- `audits/`: 通常チェック、両審査の独立棚卸しと比較、主張単位の根拠リンク、コールドレビュー、第三者最終審査、三者の実行履歴を記事ごとに外部化するJSON監査記録。
...
6. 通常チェック後の最新版本文だけを、文脈を継承しない1つの独立実行へ渡し、チェック仕様にある正本の短い依頼で1回コールドレビューする。担当は仕様が想定していない問題候補の発見だけを行う。
7. 通常チェック担当が全指摘候補を根拠確認して `採用`、`不採用`、`保留` に判定し、問題確認、必要変更、影響範囲、実施変更、残存リスクを構造化して記録し、採用指摘だけを本文へ反映する。
8. 問題候補を判定し、採用が1件以上ある場合だけ修正版の最新版全文を再検査する。保留がなければstatus `review_ready`、checked `false` として第三者の最終審査へ渡す。
9. 第三者の最終審査担当が `prompts/final_review_spec_v1.md` に従い、監査記録を見ない独立棚卸しと問題探索を先に固定する。その後、通常チェックとの棚卸し比較、全target・relation、全コールドレビュー候補、主張単位の根拠リンクを個別判定する。`PASS` の場合だけ調整役がchecked状態へ同期し、`REJECT` は完了した監査としてneeds_review状態で保存する。
10. GitHub Actionsは変更されたneeds_review/review_ready/checked/final記事について、対応するv2監査ファイル、本文ハッシュ、実行・文脈分離、盲検出力、全件判定、棚卸し比較、主張単位の根拠リンク、最終状態を検証する。既存の未変更v1監査は互換読み込みする。
...
通常チェック後は、文脈を継承しない1つの独立実行へfront matterを除いた最新版本文とチェック仕様にある正本の短い依頼だけを渡して1回コールドレビューし、通常チェック側で全指摘候補を採用・不採用・保留に判定してください。
採用が1件以上ある場合だけ修正版の最新版全文を再検査し、全監査対象、独立棚卸し候補、根拠台帳、コールドレビューと解決結果を監査ファイルへ保存してください。通常チェック側はreview_ready、checked falseまでとしてください。
その後、通常チェックとコールドレビューのどちらも担当していない別実行が prompts/final_review_spec_v1.md に従って全件を最終審査し、PASSの場合だけcheckedへ同期してください。queue/words.csv と logs/ を更新してください。
```
...
通常チェック後に1回の文脈非継承コールドレビューを行い、通常チェック側で全指摘候補を判定して必要な修正を行ってください。採用が1件以上ある場合だけ修正版の最新版全文を再検査し、全監査結果と根拠台帳をauditsへ外部化してreview_readyにしてください。
通常チェックとコールドレビューのどちらも担当していない別実行が最終審査し、全対象のPASSを本文ハッシュに結び付けて記録した場合だけ checked にし、queue/words.csv と logs/ を更新してください。
```

生成時は通常チェック、外部監査記録、1回の文脈非継承コールドレビュー、指摘の判定・修正、採用修正がある場合の全文再検査、第三者の最終審査まで行う。通常チェック側は `review_ready` までとし、第三者の `PASS` とcontent audit検証がそろった場合だけ `checked` になる。
