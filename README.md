...
5. 文脈を継承しない2つの新しい会話または独立実行へ、front matterを除いた同一の最新版本文と「英単語解説として問題がないか、記事全体を横断して内容上の問題を前提なしで指摘してください。各文の正誤だけでなく、語義の分け方・境界・重複と、学習者が説明から誤った一般化をしないかも確認してください。」という短い依頼だけを渡し、コールドレビューを2回行う。生成仕様、チェック仕様、過去の指摘、差分、queue、logs、既存判定、他方のレビュー結果は渡さない。
6. 元の作業実行が、相互に結果を見せない2回のコールドレビューから得た候補を統合し、各候補を現行仕様と信頼できる資料で検証して `採用`、`不採用`、`保留` に判定する。片方だけが挙げた候補も捨てず、採用指摘だけを本文へ反映し、保留があれば `needs_review` にする。
7. コールドレビューの指摘を1件以上採用した場合は、修正版の最新版全文に対して内容監査、双方向照合、語義境界・学習者の誤った一般化の監査、発音監査、書式監査を再実行する。採用が0件の場合は全文再検査を省略する。両方のレビューで問題候補が0件なら日付ログへ「2回のコールドレビューでは問題候補なし」、問題候補はあるが採用0件なら「コールドレビューの採用0件につき全文再検査省略」と記録する。
...
- `prompts/check_spec_v5.md`: 校正、独立構文マトリクス、ゼロベース棚卸し、品詞・構文境界監査、相互に結果を見せない2回のコールドレビュー、両方の指摘の統合・判定・修正、全文再検査、本文書式、完了条件をすべて含む現行完全版チェック仕様。通常チェックと後続判定・再検査を管理する実行だけが読み、コールドレビュー担当には渡さない。
...
6. 通常チェック後の同一最新版本文だけを、相互に結果を見せない2つの文脈非継承独立実行へ渡し、短い依頼で2回コールドレビューする。
7. 元の作業実行が全指摘候補を根拠確認して `採用`、`不採用`、`保留` に判定し、採用指摘だけを本文へ反映する。
8. 両方の候補を統合・判定し、採用が1件以上ある場合だけ、語義境界と誤った一般化を含めて修正版の最新版全文を再検査する。採用が0件の場合は全文再検査を省略し、問題候補の有無に応じた結果を `logs/` に記録する。
...
通常チェック後は、相互に結果を見せない2つの文脈非継承独立実行へfront matterを除いた同一の最新版本文と短い依頼だけを渡して2回コールドレビューし、両方の全指摘候補を統合して採用・不採用・保留に判定してください。
採用が1件以上ある場合だけ、語義境界と学習者の誤った一般化を含めて修正版の最新版全文を再検査してください。採用が0件の場合は全文再検査を省略し、問題候補の有無に応じた結果をlogsへ記録してください。2回のレビューと候補統合が完了し、保留がなく、最終状態に対する全検証に成功した場合だけ statusをchecked、checkedをtrueとして確定してください。
...
通常チェック後に相互に結果を見せない2回の文脈非継承コールドレビューを行い、両方の全指摘候補を統合・判定して必要な修正を行ってください。採用が1件以上ある場合だけ、語義境界と学習者の誤った一般化を含めて修正版の最新版全文を再検査してください。採用が0件の場合は全文再検査を省略し、問題候補の有無に応じた結果をlogsへ記録してください。
保留がなく全検証に成功した場合だけ checked にし、queue/words.csv と logs/ を更新してください。
```

生成時は通常チェック、相互に結果を見せない2回のコールドレビュー、両方の指摘の統合・判定・修正、採用修正がある場合の全文再検査まで行うため、全工程に合格した通常の完成状態は `checked` になる。2回のうち一方でも未実施、独立条件未達、候補未統合・未判定、保留あり、または必要な再検査未完了の場合は `needs_review` または未チェック状態にする。
