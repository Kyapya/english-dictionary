...
7. コールドレビューが問題候補を1件以上返した場合は、採用が0件でも、判定・修正後の最新版全文に対して内容監査、双方向照合、発音監査、書式監査を再実行する。問題候補が0件の場合は全文再検査を省略し、日付ログへ「コールドレビューでは問題候補なし」と記録する。
8. 必要な候補判定・修正・全文再検査が完了して保留が0件になった後、`queue/words.csv`、front matter、日付ログを最終状態へ同期する。その状態でstatusを `checked`、checkedを `true` とし、`scripts/validate_entry.py`、`scripts/validate_repository.py`、全単体テストを実行する。すべて成功した場合だけchecked状態を確定し、失敗した場合は `needs_review`、`checked: false` に戻して理由を記録する。
...
8. 問題候補が1件以上ある場合は、採用が0件でも判定・修正後の最新版全文を再検査する。問題候補が0件の場合は全文再検査を省略し、その結果を `logs/` に記録する。
...
問題候補が1件以上ある場合は、採用が0件でも最新版全文を再検査してください。問題候補が0件の場合は全文再検査を省略し、その結果をlogsへ記録してください。保留がなく、最終状態に対する全検証に成功した場合だけ statusをchecked、checkedをtrueとして確定してください。
...
通常チェック後に文脈非継承のコールドレビューと全指摘候補の判定・必要な修正を行い、問題候補が1件以上ある場合だけ最新版全文を再検査してください。問題候補が0件の場合は全文再検査を省略し、その結果をlogsへ記録してください。
