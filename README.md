...
4. 通常チェック担当が現行完全版 `prompts/check_spec_v5.md` だけを全文読み、生成時の分類を再利用しないゼロベース監査を行う。`scripts/content_audit.py build` で記事から全監査対象を生成し、各対象の判定、独立棚卸し候補、収録・除外理由、根拠台帳を `audits/` へ外部化する。この担当は最終合否を決めない。
...
6. 通常チェック担当がコールドレビューの各候補を現行仕様と信頼できる資料で検証し、`採用`、`不採用`、`保留` に判定する。採用指摘だけを本文へ反映し、保留があれば `needs_review` にする。
...
8. 通常チェック担当とコールドレビュー担当のどちらとも異なる実行が、`prompts/final_review_spec_v1.md` を全文読み、修正後本文、全監査対象、全独立棚卸し候補、根拠台帳、コールドレビューと解決結果を一つずつ判定する。本文は修正せず、本文ハッシュに結び付いた `PASS` または `REJECT` だけを記録する。
9. `PASS` の場合だけ、調整役が判定を変更せず `queue/words.csv` とfront matterをstatus `checked`、checked `true` へ同期する。`REJECT` の場合は通常チェック側が修正し、監査記録を更新して別実行の最終審査を再度受ける。
...
12. GitHub ActionsはAI生成・内容判断を行わず、PR内の記事形式、queueとの整合、本文ハッシュ、三者分離、全監査対象・全候補の判定、根拠参照、最終 `PASS` の完全性を再検証する。
...
- `prompts/final_review_spec_v1.md`: 通常チェック・コールドレビューとは別の実行が、全監査対象、独立棚卸し、根拠台帳、コールドレビュー解決結果を個別判定して最終合否を出す仕様。
...
- `scripts/content_audit.py`: 本文から汎用的な監査対象一覧と本文ハッシュを生成し、三者の分離、全件判定、根拠台帳、コールドレビュー解決、最終 `PASS` を検証する。
- `audits/`: 通常チェック、独立棚卸し、根拠台帳、コールドレビュー、第三者最終審査を記事ごとに外部化するJSON監査記録。
...
5. 形式検査に通ったら、通常チェック担当は `prompts/check_spec_v5.md` だけを読み、既存本文の校正と、生成時の分類を再利用しない独立したゼロベース棚卸しを行う。本文から全監査対象を生成し、全件判定、独立棚卸し候補、根拠台帳を `audits/` へ保存する。
6. 通常チェック後の最新版本文だけを、文脈を継承しない1つの独立実行へ渡し、チェック仕様にある正本の短い依頼で1回コールドレビューする。担当は仕様が想定していない問題候補の発見だけを行う。
7. 通常チェック担当が全指摘候補を根拠確認して `採用`、`不採用`、`保留` に判定し、採用指摘だけを本文へ反映する。
8. 問題候補を判定し、採用が1件以上ある場合だけ修正版の最新版全文を再検査する。保留がなければstatus `review_ready`、checked `false` として第三者の最終審査へ渡す。
9. 第三者の最終審査担当が `prompts/final_review_spec_v1.md` に従い、全監査対象、全独立候補、全コールドレビュー候補、根拠を個別判定する。`PASS` の場合だけ調整役がchecked状態へ同期し、全機械検証を実行する。
10. GitHub Actionsは変更されたchecked/final記事について、対応する監査ファイル、本文ハッシュ、全件判定、三者分離、根拠参照、最終 `PASS` を検証する。
