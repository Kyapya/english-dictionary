...
10. GitHub Actionsは変更されたneeds_review/review_ready/checked/final記事について、対応するv3監査ファイル、semantic gate、追記型履歴、本文revision、構造化レビュー原出力との項目単位一致、実行・文脈分離、盲検出力、全件判定、棚卸し比較、scope anchor、blast radius、主張単位の根拠リンク、高リスク根拠、最終状態を検証する。互換モードでもsemantic gateのない旧v3をchecked/finalの根拠にはしない。
