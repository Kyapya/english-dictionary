...
7. コールドレビューの指摘を1件以上採用した場合は、修正版の最新版全文に対して内容監査、双方向照合、発音監査、書式監査を再実行する。採用が0件の場合は全文再検査を省略する。問題候補も0件なら日付ログへ「コールドレビューでは問題候補なし」、問題候補はあるが採用0件なら「コールドレビューの採用0件につき全文再検査省略」と記録する。
8. 必要な候補判定・修正と、採用がある場合の全文再検査が完了して保留が0件になった後、`queue/words.csv`、front matter、日付ログを最終状態へ同期する。その状態でstatusを `checked`、checkedを `true` とし、`scripts/validate_entry.py`、`scripts/validate_repository.py`、全単体テストを実行する。すべて成功した場合だけchecked状態を確定し、失敗した場合は `needs_review`、`checked: false` に戻して理由を記録する。
...
8. 採用が1件以上ある場合だけ修正版の最新版全文を再検査する。採用が0件の場合は全文再検査を省略し、問題候補の有無に応じた結果を `logs/` に記録する。
9. 保留がなく必要な工程を完了した後、front matter、`queue/words.csv`、`logs/` を最終状態へ同期し、checked状態に対して全機械検証を実行する。成功時だけchecked状態を確定し、失敗時は `needs_review`、`checked: false` に戻す。
...
採用が1件以上ある場合だけ修正版の最新版全文を再検査してください。採用が0件の場合は全文再検査を省略し、問題候補の有無に応じた結果をlogsへ記録してください。保留がなく、最終状態に対する全検証に成功した場合だけ statusをchecked、checkedをtrueとして確定してください。
...
通常チェック後に文脈非継承のコールドレビューと全指摘候補の判定・必要な修正を行い、採用が1件以上ある場合だけ修正版の最新版全文を再検査してください。採用が0件の場合は全文再検査を省略し、問題候補の有無に応じた結果をlogsへ記録してください。
保留がなく全検証に成功した場合だけ checked にし、queue/words.csv と logs/ を更新してください。
```

生成時は通常チェック、コールドレビュー、指摘の判定・修正、採用修正がある場合の全文再検査まで行うため、全工程に合格した通常の完成状態は `checked` になる。コールドレビュー未実施、候補未判定、保留あり、または再検査未完了の場合は `needs_review` または未チェック状態にする。
