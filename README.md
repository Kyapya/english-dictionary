...
- `scripts/content_audit.py`: 本文から全targetと絞り込んだrelation、本文ハッシュを生成し、追記型review cycle、レビュー原出力、実行履歴、盲検最終棚卸しの固定、棚卸し比較、全件判定、主張単位の根拠リンク、コールドレビューの構造化解決、最終 `PASS` または `REJECT` を検証する。
- `audits/`: 通常チェック、両審査の独立棚卸しと比較、主張単位の根拠リンク、コールドレビュー、第三者最終審査、三者の実行履歴を記事ごとに外部化するJSON監査記録。`runs/` に各ステージの原出力、`history/` に旧監査原文、`escaped_defect_taxonomy.json` に汎用の見逃し分類を置く。
...
10. GitHub Actionsは変更されたneeds_review/review_ready/checked/final記事について、対応するv3監査ファイル、追記型履歴、本文revision、レビュー原出力、実行・文脈分離、盲検出力、全件判定、棚卸し比較、主張単位の根拠リンク、高リスク根拠、最終状態を検証する。既存の未変更v1/v2監査は互換読み込みする。
