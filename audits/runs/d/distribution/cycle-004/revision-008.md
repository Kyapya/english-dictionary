...
資料によって米語の第2音節を /ə/ または /ɪ/ と表記するが、これは辞書の表記方針や弱化を地域差そのものと同一視しない方がよい。いずれも4音節で、第3音節 /ˈbjuː/ に主強勢がある。辞書では第1音節 /ˌdɪs/ に第二強勢が示されることがあるが、実際の発話では主強勢ほど強くなく、語末の第4音節 /ʃən/ には強勢を置かない。  
...
【日本語訳・定義】Python packaging で、project（開発対象）から作られ、利用者が配布・インストールできるソフトウェアまたはリリース単位を distribution package、文脈により distribution と呼ぶ用法。project はソースコードやメタデータを含む開発対象、release は特定バージョンのスナップショット、distribution package はそのreleaseを利用者がインストールできる形にした配布・インストール対象である。distribution archive はdistribution packageを表す物理的な配布ファイルで、source distribution（sdist）やwheelなどがこれに当たる。import package はPythonコードから読み込むモジュール・パッケージで、distribution package と同じ「package」と呼ばれることがあるが別概念である。  

【頻度】〈2/10〉  

【レジスター/領域】コンピューター・IT・Python packaging。一般的なLinux distributionとは異なり、Pythonプロジェクトのビルド・配布・インストールを説明する技術文書で使われる。  

【文法パターン】a distribution package in Python packaging＝Python用の配布パッケージ／a distribution package from 〈a project〉＝〈プロジェクト〉から作られた配布パッケージ／build a distribution package＝配布パッケージをビルドする／publish or upload a distribution archive to 〈an index〉＝配布アーカイブを〈インデックス〉へ公開・アップロードする  
...
【語法・注意】Python packagingでは、project、distribution package、distribution archive、import packageは役割が異なる。projectは開発対象、distribution packageは利用者がインストールするソフトウェアまたはリリース、distribution archiveはその配布物を表す物理ファイル、import packageは実行時に読み込むPythonのモジュール・パッケージである。単語単独の distribution は文脈によりLinux distributionやPython distribution packageを指し得るため、技術文書では distribution package と明示する方が安全である。  
