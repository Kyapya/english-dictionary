...
【日本語訳・定義】Python packaging で、project（開発対象）やそのreleaseを利用者が配布・インストールできる形にしたものを distribution package、文脈により distribution と呼ぶ用法。PyPAの解説では、distribution package はインストール可能なソフトウェアを指し、一般的な説明では project とほぼ同義に扱われることがある一方、特定のversioned archive fileを指す用法もある。ここでは project を開発対象、release を特定バージョンのスナップショット、distribution package をそのreleaseを配布するためのversioned archiveまたはインストール対象、distribution archive をその物理ファイルとして整理する。両者は別の抽象階層として常に対立するのではなく、文脈により同じ配布アーカイブを指し得る。  
...
例: One distribution package can provide more than one import package.  
訳: 一つの配布パッケージが複数のimportパッケージを提供することもある。  
...
【語法・注意】Python packagingでは、project、release、distribution package、distribution archive、import packageは役割が異なる。projectは開発対象、releaseは特定バージョン、distribution packageはそのreleaseの配布・インストール対象、distribution archiveは物理ファイルを強調する呼び方で、文脈により両者が同じアーカイブを指す。source distribution（sdist）はwheelなどのbuilt distributionとは異なり、インストール時またはwheel作成前にビルド工程を要することがある。import packageは実行時に読み込むPythonのモジュール・パッケージである。単語単独の distribution は文脈によりLinux distributionやPython distribution packageを指し得るため、技術文書では distribution package と明示する方が安全である。  

【類義語】

・package  
定義: Python packagingでdistribution packageまたはimport packageを指し得る短縮形・総称。どちらを指すかは文脈で決まり、単独では曖昧になりやすい。  
頻度: 〈6/10〉  
違い: distribution packageより短く一般的だが、import packageやLinuxのパッケージも指し得るため、曖昧さを避ける技術文書ではdistribution packageまたはimport packageと明示する。  
