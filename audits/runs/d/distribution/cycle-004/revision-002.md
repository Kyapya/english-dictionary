...
【日本語訳・定義】コンピューター分野、特にLinuxなどで、複数のソフトウェア部品を組み合わせ、利用者が一まとまりとして導入・利用できる形にした配布版。Linuxに限らずBSDやTeXなどの関連ソフトウェア一式にも使う。Python packaging では distribution は、プロジェクトを配布・インストールできる単位として扱う用法で、distribution package（配布パッケージ）やその配布アーカイブを指し、この意味とは区別される。特に Linux distribution は、Linuxカーネルを基盤に各種ソフトウェアや管理ツールを組み合わせたOSの一形態を指す。  

【頻度】〈3/10〉  

【レジスター/領域】コンピューター・IT。特にLinuxなどのオープンソースOSの文脈でよく現れるが、BSDやTeXなどのソフトウェア一式にも使う専門用法。Python packaging の用法は、project、distribution package、配布アーカイブの関係を区別して扱う別の専門用法である。  
...
用途: Python packaging で、プロジェクトを利用者が配布・インストールできる形にまとめた配布パッケージを表す。distribution package は import package（importされるパッケージ）とは別概念で、配布アーカイブはその配布物の形式の一つである。  
例: The project builds a distribution package for installation.  
訳: そのプロジェクトはインストール用の配布パッケージをビルドする。  
...
【語法・注意】この語義では、主にLinuxなどで distribution 自体が「配布される一まとまりのソフトウェア構成」を指すため可算。a distribution, several distributions のように使う。Linux文脈では distro という短縮形も広く使われる。software distribution は本語義では配布されるソフトウェア一式を指し、ソフトウェアを市場へ配布する活動・仕組みは語義3の business distribution と区別する。Python packaging の distribution package は、projectを配布・インストールする単位であり、import packageとは別概念である。  
