...
【日本語訳・定義】Python packaging で、project（開発対象）から作られ、利用者が配布・インストールできるソフトウェアまたはリリース単位を distribution package、文脈により distribution と呼ぶ用法。PyPAの用語では、distribution package はインストール対象となるソフトウェアやリリースを指し、文脈によっては特定バージョンの配布ファイルを指す。distribution archive はその配布物の物理的なファイルで、source distribution（sdist）やwheelなどがこれに当たる。import package はPythonコードから読み込むモジュール・パッケージで、distribution package と同じ「package」と呼ばれることがあるが別概念である。  

【頻度】〈2/10〉  

【レジスター/領域】コンピューター・IT・Python packaging。一般的なLinux distributionとは異なり、Pythonプロジェクトのビルド・配布・インストールを説明する技術文書で使われる。  

【文法パターン】a distribution package in Python packaging＝Python用の配布パッケージ／a distribution package＝配布用パッケージ／build/publish a distribution package＝配布パッケージをビルド・公開する  
