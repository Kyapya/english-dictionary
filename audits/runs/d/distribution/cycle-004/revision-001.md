...
【文法パターン】the distribution of 〈人・物・量〉 across/over 〈範囲〉＝～が…にどう分布しているか／the distribution of 〈人・物・量〉 among 〈集団〉＝～が集団内でどう割り振られているか／a 〈normal/probability/frequency〉 distribution＝特定種類の統計分布／the distribution of 〈values/scores〉 is skewed＝～の分布は偏っている／X follows/has a 〈normal/probability〉 distribution＝Xは…の分布に従う  
...
【文法パターン】make a distribution to 〈investors/beneficiaries〉＝投資家・受益者へ分配金を支払う／receive a distribution from 〈fund/trust/estate/account〉＝ファンド・信託・遺産・口座から分配を受ける／a 〈cash/annual/final〉 distribution＝現金・年次・最終の分配  
...
【日本語訳・定義】コンピューター分野、特にLinuxなどで、複数のソフトウェア部品を組み合わせ、利用者が一まとまりとして導入・利用できる形にした配布版。Linuxに限らずBSDやTeXなどの関連ソフトウェア一式にも使う。Python packaging では distribution は通常、インストール可能なプロジェクトまたはその配布アーカイブを指し、この意味とは区別される。特に Linux distribution は、Linuxカーネルを基盤に各種ソフトウェアや管理ツールを組み合わせたOSの一形態を指す。  

【頻度】〈3/10〉  

【レジスター/領域】コンピューター・IT。特にLinuxなどのオープンソースOSの文脈でよく現れるが、BSDやTeXなどのソフトウェア一式にも使う専門用法。Python packaging の用法は、インストール可能なプロジェクトや配布アーカイブを指す別の専門用法である。  

【文法パターン】a Linux/GNU/Linux distribution＝Linux系のディストリビューション／a software distribution＝文脈によりソフトウェアの配布行為・仕組みまたは一まとまりの配布版／a Linux distribution based on 〈system/project〉＝～を基盤にしたディストリビューション  
...
用途: 文脈により、ソフトウェアを配布する行為・仕組み、または一まとまりの配布版を表す。ここでは後者を指す。  
...
【語法・注意】この語義では、主にLinuxなどで distribution 自体が「配布される一まとまりのソフトウェア構成」を指すため可算。a distribution, several distributions のように使う。Linux文脈では distro という短縮形も広く使われる。software distribution は「ソフトウェアを配布する行為・仕組み」を指すこともあり、必ずしも配布版を意味しない。ここでの a distribution は配布される製品・OS構成そのものに焦点を置く用法で、語義3のビジネス上の流通活動とも区別する。  
