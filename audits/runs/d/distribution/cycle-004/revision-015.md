...
資料によって米語の第2音節を /ə/ または /ɪ/ と表記するが、これは辞書の表記方針や弱化の実現を地域差そのものと同一視しない方がよい。ここで示した表記では4音節として数え、第3音節 /ˈbjuː/ に主強勢がある。辞書では第1音節 /ˌdɪs/ に第二強勢が示されることがあるが、実際の発話では主強勢ほど強くなく、語末音節には強勢を置かない。  
...
【日本語訳・定義】Python packaging で、project（開発対象）やそのreleaseを配布・インストールするための versioned archive file を distribution package、文脈により distribution と呼ぶ用法。PyPAの解説では、distribution package はインストール可能なソフトウェアを指し、一般的な説明では project とほぼ同義に扱われることもあるが、特定の配布ファイルを指す用法もある。ここでは project を開発対象、release を特定バージョンのスナップショット、一つのreleaseが含み得る個別のdistributionをsdistやwheelなどの配布形式、その個別のversioned archive fileをdistribution packageとして整理する。distribution archiveは同じ配布物の物理ファイル性を強調する呼称であり、distribution packageと別の配布容器ではない。  
...
【語法・注意】Python packagingでは、project、release、distribution package、distribution archive、import packageは役割が異なる。projectは開発対象、releaseは特定バージョン、一つのreleaseは一つ以上の個別distribution（sdistやwheelなど）を含み得る。distribution packageはその個別distributionのversioned archive、distribution archiveは同じ物理ファイル性を強調する呼び方である。source distribution（sdist）はソース配布アーカイブで、wheelなどのbuilt distributionとは異なり、インストール時またはwheel作成前にビルド工程を要することがある。distribution package名・project名とimport package名は一致するとは限らず、PillowがPILを提供するように異なることがある。一つのdistribution packageが複数のimport packageを提供することもあり、package単独では対象を特定できない。import packageは実行時に読み込むPythonのモジュール・パッケージである。単語単独の distribution は文脈によりLinux distributionやPython distribution packageを指し得るため、技術文書では distribution package と明示する方が安全である。  
...
例: The package is available on the Python Package Index.  
訳: そのパッケージはPython Package Indexで入手できる。  