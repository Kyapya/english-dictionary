...
Stage: `checker_recheck/round-12/frame-relation`

Run this request in an independent reviewer context. Do not read another pass request or response. Save one raw JSON response to `frame-relation.stage1.response.json`. Include pass_id and reviewer metadata with `mode: "handoff"`, your actual `declared_model`, `ingested_by: "human"`, and `agent_id: "/root/r12_frame_relation".

This is blind stage 1. Do not request or infer the alignment key. Keep this same reviewer identity/model available for stage 2 after the stage-1 checkpoint is published.
