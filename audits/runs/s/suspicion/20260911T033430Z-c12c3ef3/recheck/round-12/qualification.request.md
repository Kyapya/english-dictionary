...
Stage: `checker_recheck/round-12/qualification`

Run this request in an independent reviewer context. Do not read another pass request or response. Save one raw JSON response to `qualification.response.json`. Include pass_id and reviewer metadata with `mode: "handoff"`, your actual `declared_model`, `ingested_by: "human"`, and `agent_id: "/root/r12_qualification".
