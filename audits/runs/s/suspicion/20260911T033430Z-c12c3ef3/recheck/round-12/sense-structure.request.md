...
Stage: `checker_recheck/round-12/sense-structure`

Run this request in an independent reviewer context. Do not read another pass request or response. Save one raw JSON response to `sense-structure.response.json`. Include pass_id and reviewer metadata with `mode: "handoff"`, your actual `declared_model`, `ingested_by: "human"`, and `agent_id: "/root/r12_sense_structure".
