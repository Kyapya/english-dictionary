...
Stage: `checker_passes/frame-relation`/blind-stage1
...
Save the raw response at `responses/frame-relation.stage1.response.raw.json`. Include the exact `pass_id` and a top-level `reviewer` object with `mode: "handoff"`, your actual `declared_model`, `ingested_by: "human"`, and a unique non-empty `agent_id`. Use a different reviewer identity for each of the seven passes.

This is frame-relation blind stage 1. Keep the same reviewer for stage 2 after the sealed record and exact stage-2 request are generated.
