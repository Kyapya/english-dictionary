...
Save the raw response at `responses/sense-structure.response.raw.json`. Include the exact `pass_id` and a top-level `reviewer` object with `mode: "handoff"`, your actual `declared_model`, `ingested_by: "human"`, and a unique non-empty `agent_id`. Use a different reviewer identity for each of the seven passes.
