...
Stage: `checker_passes/example-attribution`/blind-stage1
...
Save the raw response at `responses/example-attribution.stage1.response.raw.json`. Include the exact `pass_id` and a top-level `reviewer` object with `mode: "handoff"`, your actual `declared_model`, `ingested_by: "human"`, and a unique non-empty `agent_id`. Use a different reviewer identity for each of the seven passes.

This is blind stage 1 only. Do not infer or use example ownership; stage 2 follows after the sealed record is produced.
...
```json
{
  ...
  "normalized_input_sha256": "34f529f3df5e9c7b605be5d987049381c6665a94be1360b90962ceb661d14a00"
}
```
