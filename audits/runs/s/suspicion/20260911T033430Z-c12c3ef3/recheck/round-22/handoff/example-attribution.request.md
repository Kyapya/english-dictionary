...
Run this request in its own fresh independent agent/session. Do not read earlier round findings or discuss other checker passes. Save one top-level response as `example-attribution.response.raw.json` with the exact pass_id, reviewer metadata (`mode`: `handoff`, actual `declared_model`, `ingested_by`: `human`, unique non-empty `agent_id`), and required fields.
...
```json
{
  ...
  "normalized_input_sha256": "e5ed071f2de83a69ae418ca9ffe954779cf1ad11c4ca4ab6c01f82443bc28628"
}
```
