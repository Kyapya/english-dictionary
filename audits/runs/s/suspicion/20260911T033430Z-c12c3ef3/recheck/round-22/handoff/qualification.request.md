...
Run this request in its own fresh independent agent/session. Do not read earlier round findings or discuss other checker passes. Save one top-level response as `qualification.response.raw.json` with the exact pass_id, reviewer metadata (`mode`: `handoff`, actual `declared_model`, `ingested_by`: `human`, unique non-empty `agent_id`), and required fields.
