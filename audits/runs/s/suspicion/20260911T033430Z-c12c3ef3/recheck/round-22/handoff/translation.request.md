...
Run this request in its own fresh independent agent/session. Do not read earlier round findings or discuss other checker passes. Save one top-level response as `translation.response.raw.json` with the exact pass_id, reviewer metadata (`mode`: `handoff`, actual `declared_model`, `ingested_by`: `human`, unique non-empty `agent_id`), and required fields.
...
```json
{
  ...
  "normalized_input_sha256": "cc0b2ba845b888a502bbc572b7cd78b898a4409538f30f895256446f0e562803"
}
```
