...
Run this request in its own fresh independent agent/session. Do not read earlier round findings or discuss other checker passes. Save one top-level response as `sense-structure.response.raw.json` with the exact pass_id, reviewer metadata (`mode`: `handoff`, actual `declared_model`, `ingested_by`: `human`, unique non-empty `agent_id`), and required fields.
...
```json
{
  ...
  "normalized_input_sha256": "1e575d711776a4a73727eac95f6009c443c9d956263ed97b2515751772756463"
}
```
