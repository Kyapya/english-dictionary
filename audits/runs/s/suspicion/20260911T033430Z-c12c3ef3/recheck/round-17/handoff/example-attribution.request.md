...
Save exactly one JSON response as `example-attribution.response.raw.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty `agent_id`. Each checker pass must use a different agent_id.
## Prompt
...
```json
{
  ...
  "input_sections": {
    "sense_structure": [
      ...
      },
      {
        ...
        "definition": "十分な証拠はないが、ある事実や状況が本当なのではないかと感じる推測・予感に焦点がある。a suspicion that ... の that 節が犯罪・不正を述べる場合は、語義1の「犯罪・不正の容疑」と重なることもある。この用法では個々の考えを表す可算形が典型。可算・不可算は意味だけで一律に決まらず、構文にも左右される。"
      },
      {
        "sense_id": "sense:004",
        "line": 186,
  ...
  "normalized_input_sha256": "ea17fcc5b625a2f8f4c6ffba1a743e872bd3f88b548c39edbc1bb16650109561"
}
```
