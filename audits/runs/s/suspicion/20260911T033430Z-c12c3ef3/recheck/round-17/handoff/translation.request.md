...
Save exactly one JSON response as `translation.response.raw.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty `agent_id`. Each checker pass must use a different agent_id.
## Prompt
...
```json
{
  ...
  "normalized_input_sha256": "831c179f40197776ff9ba26b19a380c8733a18b685009bc591057b3ab0c872fd"
}
```
