...
Stage: `checker_passes/frame-relation` round 26 (blind-stage1)
...
Save one raw JSON response at `audits/runs/s/suspicion/20260911T033430Z-c12c3ef3/recheck/round-26/responses/frame-relation.response.raw.json`. Include `pass_id`, your actual `declared_model`, `ingested_by: "human"`, and a unique non-empty `agent_id` in the top-level `reviewer` object. Each pass must use a different reviewer identity. This is blind stage 1; keep this reviewer for stage 2 after the sealed record and exact stage-2 request are generated.
