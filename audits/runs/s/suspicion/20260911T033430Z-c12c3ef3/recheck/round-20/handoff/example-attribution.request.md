...
Run this request in its own independent agent/session. Do not read earlier round findings or discuss other checker passes. Save one top-level response as `example-attribution.response.raw.json` with the exact pass_id, a reviewer object (`mode`: `handoff`, actual `declared_model`, `ingested_by`: `human`, and a unique non-empty `agent_id`), and the required response fields.
