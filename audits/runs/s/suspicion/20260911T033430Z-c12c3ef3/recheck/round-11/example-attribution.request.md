...
Save exactly one JSON response as `example-attribution.blind-record.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty `agent_id`. Each checker pass must use a different agent_id.
## Prompt
...
```json
{
  ...
  "normalized_input_sha256": "c712ad0c33a97d1900e2c885b6582f980239b4393642a5a7c110e4d5650d4210"
}
```
