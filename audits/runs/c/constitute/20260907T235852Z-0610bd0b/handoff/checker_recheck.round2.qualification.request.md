...
Save exactly one JSON response as `../recheck_round2/qualification.response.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty unique `agent_id`.
...
```json
{
  ...
  "normalized_input_sha256": "0fbeefa81995c18d6a1898c0b73c3c40889256dce8263ab40dc4c0ebf6757f08"
}
```
