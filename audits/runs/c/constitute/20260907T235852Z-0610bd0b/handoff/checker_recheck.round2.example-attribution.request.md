...
Run this request in its own independent subagent/session. The seven round-2 requests may run concurrently; do not concatenate them or reuse one subagent for multiple passes.

Save exactly one JSON response as `../recheck_round2/example-attribution.response.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty unique `agent_id`.
...
```json
{
  ...
  "source_artifact_sha256": "f63969a81cebb477471a1f01a3a087b0a9c85d94795a22dc1626229395687f76",
  "normalized_input_sha256": "555d5cf5f0a3eec74eb8eaa5f4825a23610db22cd4ce1eac8d32f0aca95d6b79"
}
```
