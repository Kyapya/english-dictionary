...
Save exactly one JSON response as `../recheck_round2/frame-relation.response.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty unique `agent_id`.

This is frame-relation stage 1 only. Keep this exact subagent available. After stage 1 is sealed, the same reviewer.agent_id and declared_model must execute the generated round-2 stage-2 request.
