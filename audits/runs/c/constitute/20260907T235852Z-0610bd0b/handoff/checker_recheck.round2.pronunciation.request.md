...
Stage: `checker_recheck/round2/pronunciation`
...
Save exactly one JSON response as `../recheck_round2/pronunciation.response.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty unique `agent_id`.
...
```json
{
  ...
  "normalized_input_sha256": "7333d5eaf910540c40f2825a96f8171ff793fcbc0b89e5e399805102adb8848f"
}
```
