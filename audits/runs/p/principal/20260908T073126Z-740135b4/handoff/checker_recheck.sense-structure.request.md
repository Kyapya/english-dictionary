...
Save exactly one JSON response as `recheck/sense-structure.response.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty `agent_id`. Each rechecked pass must use a different agent_id.
## Prompt
...
```json
{
  ...
  "normalized_input_sha256": "0412b5df39a7b247f842afcc74ea103a5a0d327eb1e11fd61ded1e3280541f2a"
}
```
