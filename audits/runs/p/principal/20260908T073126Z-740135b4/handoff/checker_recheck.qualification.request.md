...
Save exactly one JSON response as `recheck/qualification.response.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty `agent_id`. Each rechecked pass must use a different agent_id.
## Prompt
...
```json
{
  ...
  "normalized_input_sha256": "688b5fc9ec3aaf5370042a8ad6da09b9f478bafa5cb22c41f33e13a3eb19aa0d"
}
```
