...
Save exactly one JSON response as `recheck/round2/frame-relation.response.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty `agent_id`. Each round-2 reviewer agent_id must be unique and must differ from the reused reviewer IDs `/root/principal_cycle3_sense` and `principal-cycle3-pronunciation`.
This is frame-relation stage 1 only. Keep this exact reviewer available. After deterministic fan-in, the same reviewer.agent_id and declared_model must complete the generated frame-relation stage-2 request.
    ...
    "antonym_items": [
      {
        ...
        "headword": "principal",
        "sense_definition": "【日本語訳・定義】別の人・法人である `agent` に、自分のために行動する権限を与える人または法人。代理人は本人のために行動し、代理関係では `principal` が権限の源となる。米国の一般的な代理法の説明では、代理人は本人のために、かつ本人の支配の下で行動する。",
        "antonym": "agent",
