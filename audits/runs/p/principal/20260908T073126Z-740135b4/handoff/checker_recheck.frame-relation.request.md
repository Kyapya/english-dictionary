...
Save exactly one JSON response as `recheck/frame-relation.response.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty `agent_id`. Each rechecked pass must use a different agent_id.
This is frame-relation stage 1 only. Keep this exact subagent available. After stage-1 fan-in, the same reviewer.agent_id and declared_model must execute the generated frame-relation stage-2 request.
...
```json
{
  ...
  "normalized_input_sha256": "02feb3f7c018d668faa5f581ec770201cb145e3351381db3fad4b0c6f6face77"
}
```
