...
Stage: `checker_recheck/round2/qualification`
...
Save exactly one JSON response as `recheck/round2/qualification.response.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty `agent_id`. Each checker pass must use a different agent_id.
## Prompt
...
```json
{
  ...
  "normalized_input_sha256": "3d8d986842a24db6e88d756a382c04c94cd06b86549592001899dcf4fa9f0fa2"
}
```
