...
Stage: `checker_recheck/round2/sense-structure`
...
Save exactly one JSON response as `recheck/round2/sense-structure.response.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty `agent_id`. Each checker pass must use a different agent_id.
## Prompt
...
```json
{
  ...
  "normalized_input_sha256": "91338f9149397854c1650123d7e7b557efe5f08457b3382f74dad6149fbfd8f0"
}
```
