...
Stage: `checker_recheck/frame-relation`
...
Save exactly one JSON response as `recheck/frame-relation.response.json`. The top-level JSON must include the routed `pass_id` and a `reviewer` object with `mode: "handoff"`, the actual `declared_model`, `ingested_by: "human"`, and a non-empty `agent_id`. Each checker pass must use a different agent_id.
...
```json
{
  ...
  "normalized_input_sha256": "afbae4919e2eb5952c2447ca18590f14972e1208d556bc91388431e7cf5abfc2"
}
```
