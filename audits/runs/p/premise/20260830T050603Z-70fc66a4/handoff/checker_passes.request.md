  ...
  "requests": [
    {
      ...
      "input_sections": {
        "definitions": [
          ...
          {
            "line": 219,
            "text": "【日本語訳・定義】人・会社・組織などが所有・占有・使用する土地、建物、建物の一部、およびそれに付随する敷地。会社・店舗・学校・病院・工場などの場所に特によく使うが、住宅や賃貸物件などにも使える。現代英語ではこの意味を通常 `premises` という複数形で表す。単一の施設でもこの複数形を使うのが一般的で、法務・規制・不動産管理などの限定された文脈に単数の `a premise` / `one premise` が現れることもあるが、まれで法域や文書慣行に依存する。学習者が一般文で単一の場所を言うなら `a building`、`a property`、`a site` のほうが安全で自然なことが多い。  "
          },
          {
            "line": 283,
        ...
        "collocations_examples": [
          ...
          {
            "line": 358,
            "text": "訳: 2回の測定は同一条件で行われたことを前提としておこう。  "
          },
          {
            "line": 360,
    ...
    },
    {
      ...
      "input_sections": {
        "core_image": [
          ...
          {
            "line": 39,
            "text": "`premises` の「建物・敷地」という用法は歴史的には同じ語から発達したが、現代話者にとって上のコアイメージから直接推測しにくい定着義なので、語義4として別に意識する。ただし、一般用法の標準形は複数形 `premises` であり、限定的でまれな単数形 `a premise` は法域や文書慣行に依存するため、通常の言い換えとしては `a building`、`a property`、`a site` などを使う。名詞の「前提」と「前提命題」は同じ中心概念を共有し、論理学では後者の専門的な枠組みとして現れる。動詞の二つの説明も、同じ「前提を置く」という核を目的語の取り方の違いから整理したものである。  "
          }
        ],
        "sense_structure": [
          {
      ...
      "input_sections": {
        "sense_structure": [
          ...
          },
          {
            ...
            "definition": "人・会社・組織などが所有・占有・使用する土地、建物、建物の一部、およびそれに付随する敷地。会社・店舗・学校・病院・工場などの場所に特によく使うが、住宅や賃貸物件などにも使える。現代英語ではこの意味を通常 `premises` という複数形で表す。単一の施設でもこの複数形を使うのが一般的で、法務・規制・不動産管理などの限定された文脈に単数の `a premise` / `one premise` が現れることもあるが、まれで法域や文書慣行に依存する。学習者が一般文で単一の場所を言うなら `a building`、`a property`、`a site` のほうが安全で自然なことが多い。"
          },
          {
            "sense_id": "sense:005",
            "line": 283,
        ...
        "collocations_examples": [
          ...
          },
          {
            ...
            "translation": "2回の測定は同一条件で行われたことを前提としておこう。"
          }
        ]
      },
      "blind_protocol": {
