  ...
  "requests": [
    {
      ...
      "input_body_sha256": "b6e3470537f5bdb2a705ac668986878680f7a8f40716131d15a451a9865b3171",
      "input_sections": {
        "definitions": [
          {
            "line": 43,
          ...
          {
            "line": 167,
            "text": "【日本語訳・定義】論証・推論において、結論を導くための出発点として置かれる命題。三段論法では `major premise`「大前提」、`minor premise`「小前提」のように呼ぶ。語義1の「議論の土台」という意味が、論理学で論証の構成要素として具体化した用法であり、まったく別の概念ではない。ここでは学習上、`major premise` / `minor premise` などの専門語法をまとめて示す。  "
          },
          {
            "line": 217,
          ...
          {
            "line": 219,
            "text": "【日本語訳・定義】人・会社・組織などが所有・占有・使用する土地、建物、建物の一部、およびそれに付随する敷地。会社・店舗・学校・病院・工場などの場所に特によく使うが、住宅や賃貸物件などにも使える。現代英語ではこの意味を通常 `premises` という複数形で表す。単一の施設でもこの複数形を使うのが一般的だが、法務・規制・不動産管理などの限定された文脈では、単数の `a premise` / `one premise` も見られる。一般文では `a building`、`a property`、`a site` のほうが自然なことが多い。  "
          },
          {
            "line": 283,
          ...
          {
            "line": 285,
            "text": "【日本語訳・定義】理論、議論、計画、判断などを、ある考え・仮定が真または受け入れられるものとして土台にして組み立てる。語義6と同じ「前提を置く」という核を持つが、ここでは目的語Aを前提Bの上に組み立てる `A on B` の構文に焦点を置く。現代英語では、とくに受動態 `be premised on/upon ...`「～を前提としている、～に基づいている」が重要である。  "
          },
          {
            "line": 338,
          ...
          {
            "line": 340,
            "text": "【日本語訳・定義】語義5と同じ「前提を置く」という核を持つが、ここでは目的語・that節の内容そのものを前提として提示する。議論や推論を始める前に、ある命題・考えを前提として提示したり、真であるものとして仮に置いたりする。語義5の `premise A on B` では A が前提に基づいて組み立てられる対象だが、この語義では目的語・that節の内容そのものが「前提として置かれる内容」になる。  "
          }
        ],
        "collocations_examples": [
          {
    ...
    },
    {
      ...
      "input_sections": {
        "core_image": [
          ...
          {
            "line": 33,
            "text": "`premise` の中心は、「後に続く結論・判断・構成を支えるものを、先に置いて土台にする」である。議論では命題を先に置き、判断では仮定を土台にし、作品では物語を成立させる基本設定を置く。動詞では、その土台の上に理論や主張を組み立てることを表す。以下の語義5・6は別々の意味を暗記するためのものではなく、この同じ「前提を置く」という意味核を、項の取り方が異なる二つの構文として示している。  "
          },
          {
            "line": 34,
            "text": "・議論・判断の土台として置く考え → 「前提、仮定」  "
          },
          {
            "line": 35,
            "text": "・作品を成立させる土台の設定・筋立ての核 → 「基本設定、中心的な着想」  "
          },
          {
            "line": 36,
            "text": "・論理学で議論の土台となる命題 → 「前提命題」  "
          },
          {
            "line": 37,
            "text": "・理論・主張などをある考えの上に置く → 「～を…に基づかせる」  "
          },
          {
            "line": 38,
            "text": "・命題や考え自体を前提として先に置く → 「～を前提として述べる・仮定する」  "
          },
          {
            "line": 39,
            "text": "`premises` の「建物・敷地」という用法は歴史的には同じ語から発達したが、現代話者にとって上のコアイメージから直接推測しにくい定着義なので、語義4として別に意識する。ただし、一般用法の標準形は複数形 `premises` であり、限定的な単数形 `a premise` を一律に誤りとはしない。 名詞の「前提」と「前提命題」は同じ中心概念を共有し、論理学では後者の専門的な枠組みとして現れる。動詞の二つの説明も、同じ「前提を置く」という核を目的語の取り方の違いから整理したものである。  "
          }
        ],
        "sense_structure": [
          {
        ...
        "usage_notes": [
          ...
          {
            "line": 254,
            "text": "【語法・注意】この意味の `premises` は見た目が複数形で、標準的には複数動詞を取る。`These premises are ...`、`the premises have ...` のように考える。単数の建物や物件を指すときも、一般的な文では `a building`、`a property`、`a site` などのほうが自然なことが多い。ただし法務・規制・施設管理などでは `a premise`、`a single premise`、`one premise` のような単数形も見られるため、`a premise` を一律に誤用と考えない。  "
          },
          {
            "line": 283,
            "text": "5. 【他動詞・形式的】〈理論・主張・行動など〉を～という前提に置く、～に基づかせる"
          ...
          {
            "line": 365,
            "text": "【語法・注意】語義5と語義6は意味の核が連続しており、主な違いは項の取り方である。この語義は現代英語では低頻度で、学習上は語義5の `be premised on/upon ...` のほうを優先する。`premise that P` では P 自体を前提として置くのに対し、`A is premised on P` では A が P を土台として組み立てられている。項の方向が異なるので混同しない。  "
          }
        ],
        "word_formation": [
          {
          ...
          {
            "line": 26,
            "text": "・premises：文脈により、`premise` の通常の複数形（複数の前提）または、建物・敷地を指す定着した不動産用法を表す。後者は単一の場所にも使われ、見た目は複数形だが通常は複数動詞を取る。  "
          },
          {
            "line": 27,
      ...
      "input_sections": {
        "sense_structure": [
          ...
          },
          {
            ...
            "definition": "論証・推論において、結論を導くための出発点として置かれる命題。三段論法では `major premise`「大前提」、`minor premise`「小前提」のように呼ぶ。語義1の「議論の土台」という意味が、論理学で論証の構成要素として具体化した用法であり、まったく別の概念ではない。ここでは学習上、`major premise` / `minor premise` などの専門語法をまとめて示す。"
          },
          {
            "sense_id": "sense:004",
            "line": 217,
            "label": "4. 【名詞・複数形】建物・敷地、構内（特に事業所・施設など）",
            "definition": "人・会社・組織などが所有・占有・使用する土地、建物、建物の一部、およびそれに付随する敷地。会社・店舗・学校・病院・工場などの場所に特によく使うが、住宅や賃貸物件などにも使える。現代英語ではこの意味を通常 `premises` という複数形で表す。単一の施設でもこの複数形を使うのが一般的だが、法務・規制・不動産管理などの限定された文脈では、単数の `a premise` / `one premise` も見られる。一般文では `a building`、`a property`、`a site` のほうが自然なことが多い。"
          },
          {
            "sense_id": "sense:005",
            "line": 283,
            "label": "5. 【他動詞・形式的】〈理論・主張・行動など〉を～という前提に置く、～に基づかせる",
            "definition": "理論、議論、計画、判断などを、ある考え・仮定が真または受け入れられるものとして土台にして組み立てる。語義6と同じ「前提を置く」という核を持つが、ここでは目的語Aを前提Bの上に組み立てる `A on B` の構文に焦点を置く。現代英語では、とくに受動態 `be premised on/upon ...`「～を前提としている、～に基づいている」が重要である。"
          },
          {
            "sense_id": "sense:006",
            "line": 338,
            "label": "6. 【他動詞・形式的・まれ】〈命題・考え〉を前提として置く、～だと前提として述べる・仮定する",
            "definition": "語義5と同じ「前提を置く」という核を持つが、ここでは目的語・that節の内容そのものを前提として提示する。議論や推論を始める前に、ある命題・考えを前提として提示したり、真であるものとして仮に置いたりする。語義5の `premise A on B` では A が前提に基づいて組み立てられる対象だが、この語義では目的語・that節の内容そのものが「前提として置かれる内容」になる。"
          }
        ],
        "collocations_examples": [
          {
            "example_id": "ex-bc34967c85d7",
    ...
    },
    {
      ...
      "input_sections": {
        "etymology": [
          ...
          {
            "line": 22,
            "text": "`premises` の「建物・敷地」という意味は、単に「前提が複数ある」ことから生じたのではない。法律文書で先に述べられた事項や、譲渡文書で記載された対象を *premises* と呼んだ歴史を経て、そこに記載された土地・建物そのものを指すようになった。現代英語では、建物・敷地を指すときは複数形 `premises` が標準的な形である。一方、法務・規制・不動産管理などの限定された文脈では、単一の場所を指して単数の `a premise` / `one premise` も使われる。したがって「常に複数形」や「単数形は常に誤り」と一般化せず、一般用法の標準形と専門文脈の単数形を分けて覚えるのが実用的である。  "
          }
        ],
        "word_formation": [
          {
