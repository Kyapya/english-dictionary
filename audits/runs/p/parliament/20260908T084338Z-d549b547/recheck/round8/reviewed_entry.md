...
【日本語訳・定義】主にイギリスの用法で、総選挙を区切りとして成立する特定回の議会、またはその存続期間を指す。一つの `Parliament` は複数の `session` に分かれる。  
...
【語法・注意】語義1の制度としての `parliament` は選挙を越えて継続するが、この語義は総選挙を区切りとして成立する特定回の議会体とその存続期間を表す。`Parliament` と `session` は同じではなく、一つの `Parliament` は複数の `session` に分かれる。`dissolution` はその Parliament 自体を終える。  
